 ...
 -->
	...
	<div class="span3 bs-docs-sidebar " id="nav_menu" style="display:none">
		...
		<div class="teachForm  span12 " id="resultview">
			...
			<section id="teaching">
				...
				<h2 class="description" for="element_1">Customized dated bill of all students</h2><p id="li_1">
				<h4>Start Date -> <?= $_POST['sday']."/".$_POST['smonth']."/".$_POST['syear'] ;?> , End Date -> <?= $_POST['eday']."/".$_POST['emonth']."/".$_POST['eyear'] ;?> , with choices of month -> <?= $_POST['cmonth'] ?>, year -> <?= $_POST['cyear'] ?></h4>
	...
	<?php 
		...
		while($row = mysql_fetch_array($result_db)) {
			 ...
			 $sid = $row['s_id'];
			 $mpay = $grtotal;//$row['cost_month_nobasic'] ;
